...
export function useTaskLogger() {
  ...
  const getCategoryDepth = useCallback((categories: Category[], targetName: string, depth = 0): number => {
    for (const cat of categories) {
      if (cat.name === targetName) {
        return depth;
      }
      if (cat.subcategories && cat.subcategories.length > 0) {
        const found = getCategoryDepth(cat.subcategories, targetName, depth + 1);
  ...
  const recordClick = useCallback((categoryName: string, categories: Category[]) => {
    ...
    setMenuTravelDistance(prev => prev + Math.abs(currentDepth - lastClickDepthRef.current));
